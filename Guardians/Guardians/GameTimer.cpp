...
{
	if (QueryPerformanceFrequency((LARGE_INTEGER *)&m_PerformanceFrequency))    //64bit 정수값 컴퓨터가 서은ㅇ 카운터 하드웨어를 가지고 있는지 확인. 
	{
	...
	{
		// 멀티미디어 타이머 사용. 시간단위는 0.001초 ( ms - 밀리세컨 단위 ) 
...
{
	if (m_bHardwareHasPerformanceCounter)	QueryPerformanceCounter((LARGE_INTEGER*)&m_nCurrentTime);
	else									m_nCurrentTime = timeGetTime();
	...
	if (fLockFPS > 0.0f)
	{
		//이 함수의 파라메터가 0보다 크면 이 시간만큼 호출한함수를 기다리게 한다.
