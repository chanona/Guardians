...
void CNetEngine::Accept()
{
	...
	while (1)
	{
		...
		if (pNewSession->AcceptCompletion())
		{
			...
			player->SetPosition(XMFLOAT3(PLAYER_INIT_X, PLAYER_INIT_Y, PLAYER_INIT_Z));
			player->SetWorld(WORLD_MANAGER->GetWorld(WORLD_TYPE::FOREST));
			player->UpdateSector(player->GetPositionX() / SECTOR_WIDTH, player->GetPositionZ() / SECTOR_HEIGHT);
			...
			login_pkt.x = PLAYER_INIT_X;
			login_pkt.y = PLAYER_INIT_Y;
			login_pkt.z = PLAYER_INIT_Z;
			login_pkt.hp = 100;
